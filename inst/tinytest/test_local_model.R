...
expect_true(is.function(llamaR::default_local_model))

# Clear cache before each probe.
reset_cache <- function() {
    assign("initialized", FALSE,
           envir = llamaR:::.local_model_cache)
    assign("value", NULL, envir = llamaR:::.local_model_cache)
}
...
local({
    reset_cache()
    op <- options(llamaR.local_models = "definitely-not-a-real-model:0.01b")
    on.exit({
        options(op)
        reset_cache()
    }, add = TRUE)
    expect_null(llamaR::default_local_model())
})
...
local({
    reset_cache()
    op <- options(llamaR.local_models = "definitely-not-a-real-model:0.01b")
    first <- llamaR::default_local_model()
    options(llamaR.local_models = c("also-not-real", "still-not-real"))
    on.exit({
        options(op)
        reset_cache()
    }, add = TRUE)
...
if (at_home()) {
    reset_cache()
    op <- options(llamaR.local_models = c("gpt-oss:120b", "gpt-oss:20b"))
    on.exit({
        options(op)
        reset_cache()
    }, add = TRUE)
    ...
    if (any(c("gpt-oss:120b", "gpt-oss:20b") %in% available)) {
        picked <- llamaR::default_local_model()
