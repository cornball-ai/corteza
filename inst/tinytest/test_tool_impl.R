...
# Test run_r
result <- corteza:::tool_run_r(list(code = "1 + 1"))
...
result <- corteza:::tool_grep_files(list(
    pattern = "find this",
    path = tmp_dir,
    file_pattern = "*.R"
))
...
result <- corteza:::tool_write_file(list(path = file_path, content = "alpha\nbeta\n"))
expect_false(isTRUE(result$isError))
expect_true(file.exists(file_path))

result <- corteza:::tool_read_file(list(path = file_path, from = 2L, lines = 1L))
expect_false(isTRUE(result$isError))
expect_true(grepl("2 \\| beta", result$content[[1]]$text))

result <- corteza:::tool_replace_in_file(list(
    path = file_path,
    old_text = "beta",
    new_text = "gamma"
))
...
result <- corteza:::tool_list_files(list(path = tmp_dir))
expect_false(isTRUE(result$isError))
...
result <- corteza:::tool_installed_packages(list(pattern = "jsonlite", limit = 20L))
expect_false(isTRUE(result$isError))
