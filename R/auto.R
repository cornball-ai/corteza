...
auto_tracked_files <- function(dir = getwd()) {
    ...
    in_repo <- identical(
                         suppressWarnings(tryCatch(
                                                   git_run(c("rev-parse", "--is-inside-work-tree"),
                                                           path = dir)$text,
                                                   error = function(e) "")),
                         "true")
    if (in_repo) {
...
auto_spend_since <- function(session, baseline) {
    ...
    main_tok <- sum(vapply(segs, function(s) as.numeric(s$total_tokens %||% 0),
                           numeric(1)))
...
auto_check_limits <- function(state, auto) {
    ...
    if (is.finite(auto$max_minutes) && elapsed >= auto$max_minutes) {
        return(halt(sprintf("hit the time cap (%.1f of %g minutes)",
                            elapsed, auto$max_minutes)))
    }
    if (state$tool_calls >= auto$max_tool_calls) {
        return(halt(sprintf("hit the tool-call cap (%d)",
                            auto$max_tool_calls)))
    }
    ...
    if (is.finite(auto$max_tokens) && (spend$tokens %||% 0) >= auto$max_tokens) {
